...
# Add the balance in the subclass' construction method. Please note that balance 
# is not default parameter but a instance variable in CreditCard
...
# (2). Aopt a base class and its subclass 
...
# Modify the sequence (based on R2.14)
...
class Sequence(metaclass=ABCMeta):
    ...
    def count(self, val):
        ...
        return k
    def __eq__(self, other):
        # Assume they have to be equal and have the same number of elements 
        ...
        if len(self) != len(other):
            ...
            for i in range(len(self)):
                if self[i] != other[i]:
                    return False
            return True  # If you made it through, all the elements are same

##############################################################################################


# R2.23 Add the method of __lt__() to parametize Sequence class
...
# C2.29. Modify the PredatoryCreditCard class from Section 2.4.1 so that a cus-
...
# Give a minimum payment, otherwise appriase delayed fees
...
# Add gender(bool) and a float strength into every animal. Add Animal as base class.
...
# Polygon as base class and develop subclasses 
