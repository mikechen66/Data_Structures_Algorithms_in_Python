...
A binary search tree (BinarySearchTree) is a binary tree where every node in the left subtree is less than the root, 
and every node in the right subtree is of a value greater than the root. The properties of BinarySearchTree are 
...
BinarySearchTree can support the queries MINIMUM, MAXIMUM, SUCCESSOR,and PREDECESSOR and SEARCH. It allows insertion 
...
We define the successor of a node as the next node visited in an inorder tree walk. The structure of BinarySearchTree 
...
The insert procedure inserts a new node into T as a BinarySearchTree. The procedure takes T and a node z for which z.key 
...
# https://web.stanford.edu/class/cs106b/lectures/23-BinarySearchTree/slides
# https://courses.engr.illinois.edu/cs225/sp2022/resources/BinarySearchTree/
"""
...
class BinarySearchTree:
    ...
    def tree_insert(self, data):
        z = BinarySearchTreeNode(data)                                # Initialize the BinarySearchTreeNode(data)
    ...
    # Delete node z from the BinarySearchTree and maintain its property.
...
if __name__ == "__main__":
    ...
    print(binary_tree3)
    # If get_key not provided, must define __gt__ or __lt__ for objects in the BinarySearchTree.
    binary_tree4 = BinarySearchTree()
    for x in array3:
        binary_tree4.tree_insert(x)
    print(binary_tree4)
    print(binary_tree4.is_bst())
    # Binary tree that does not have the BinarySearchTree property.
